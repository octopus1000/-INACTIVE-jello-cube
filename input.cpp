...
#define BUFSIZE  512
	...
	switch (button)
  {
    case GLUT_LEFT_BUTTON:
	  ...
	  if (!g_iLeftMouseButton)
		  (memset(z_mouseDelta, 0, sizeof(int) * 2),memset(&z_userForce,0,sizeof(vec)),z_hit = 0);
	  else
		  (z_mouseOrigin[0] = x, z_mouseOrigin[1] = y);
      break;
  ...
  glPushMatrix();
  glLoadIdentity();
  gluPickMatrix((GLdouble)x, (GLdouble)(viewport[3] - y), 1.0, 1.0, viewport);//This will change matrix stack
  double aspectRatio = 1.0 * viewport[2] / viewport[3];
...
void processHits(GLint hits, GLuint buffer[]){
	...
	ptr = (GLuint *)buffer;
	z_hit = hits > 0 ? 1 : 0;
	for (i = 0; i < (int)hits; i++) {  /* for each hit  */
		(++ptr)++;//skip number of names and z1
		z = (double)*ptr / 0xffffffff;
		if ( z < zmin){//z2 < zmin
			zmin = z;
			name = *(++ptr);ptr--;
		}
	...
	if (z_hit){
		printf("name = %d, z2 = %g\n", name, zmin);
		pMAKE(name / 64, (name - name / 64 * 64) / 8, name % 8, z_hitPos);
	}		
}
