...
int z_mouseDelta[2] = { 0, 0 }, z_mouseOrigin[2] = { 0, 0 };
GLdouble z_mVMat[16], z_projMat[16];
bool z_hit = 0;//curser doesnt hit obj
vec z_hitPos, z_userForce = { 0, 0, 0 };
...
void display()
{
  ...
  // keep matrix for unproject
  glGetDoublev(GL_MODELVIEW_MATRIX, z_mVMat);
  glGetDoublev(GL_PROJECTION_MATRIX, z_projMat);
